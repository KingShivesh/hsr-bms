...
- C3 IN PROGRESS: command palette search is being extended one data type at a time. Customer search is committed and verified with a temporary QA customer; menu item search is verified against a real menu item and always navigates to Inventory. Bill/transaction search is intentionally scoped out until there is a specific bill-detail/filter/highlight destination.
